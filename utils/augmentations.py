...
class AutoAUG(Module):
    ...
    def random_jitter(
        ...
        x_raw = x * std + mean
        max_sigma = std * 0.2
        output, labels = jitter(sigma=max_sigma, random_sigma=False,frac=True)(x_raw)
    ...
    def random_scaling(self, x, max_sigma=0.5):
        ...
        std = self.cfg.DATASET.STD
        x_raw = x * std + mean
        output, labels = scaling(max_sigma=max_sigma, random=True)(x_raw)
        output = (output - mean) / std
        return output, labels
    ...
    def random_timeshift(x):
        output = TimeShift(max_shift=0.4, random=True)(x)
        return output
